...
# Creates the logistic regression object and fits it to each training set
PER_log_reg = LogisticRegression(max_iter=50000)
PER_log_reg.fit(PER_X_train, PER_y_train)
vorp_log_reg = LogisticRegression(max_iter=50000)
vorp_log_reg.fit(vorp_X_train, vorp_y_train)
bpm_log_reg = LogisticRegression(max_iter=50000)
bpm_log_reg.fit(bpm_X_train, bpm_y_train)
ws48_log_reg = LogisticRegression(max_iter=50000)
ws48_log_reg.fit(ws48_X_train, ws48_y_train)
ws_log_reg = LogisticRegression(max_iter=50000)
ws_log_reg.fit(ws_X_train, ws_y_train)

# Now print out each model's accuracy based on the each statistic's testing set
print('***ALL LOGISTIC REGRESSION SCORES***\n')
print('PER testing score= ' + str(PER_log_reg.score(PER_X_test, PER_y_test)))
print('vorp testing score= ' + str(vorp_log_reg.score(vorp_X_test, vorp_y_test)))
print('bpm testing score= ' + str(bpm_log_reg.score(bpm_X_test, bpm_y_test)))
print('ws/48 testing score= ' + str(ws48_log_reg.score(ws48_X_test, ws48_y_test)))
print('ws testing score= ' + str(ws_log_reg.score(ws_X_test, ws_y_test)))
